...
def check(ctx):
    base, logdir, _ = rotate_a_copy(ctx, 9)
    try:
        kept = rotated(logdir)
        evidence = "after nine rotations: " + (", ".join(kept) or "nothing kept")
        if len(kept) != 7:
            return ctx.failed(f"Nine rotations kept {len(kept)} old logs, not seven.", evidence)
        # delaycompress leaves the newest one plain for a day; the rest must be compressed
        plain = [f for f in kept if not f.endswith(".gz") and f != "app.log.1"]
        if plain:
            return ctx.failed("Old logs are kept uncompressed.", evidence)
        return ctx.passed("Seven rotated logs are kept, compressed.", evidence)
    finally:
