...
def check(ctx):
    ...
    try:
        ...
        if not moved_on(logdir):
            return ctx.failed("No rotation happened in the copy, so there is nothing to test yet.",
                              outputs[0])  # fmt: skip
        r = ctx.run(["su", "-s", "/bin/sh", "-c", f"echo after >> {app}", "shop"])
        listing = ctx.run(["ls", "-l", logdir]).out
        if not r.ok:
            return ctx.failed("After a rotation, shop can no longer write app.log.",
                              f"{r.err.strip()}\n{listing}")  # fmt: skip
        return ctx.passed("After a rotation, shop still writes app.log.", listing)
    finally:
