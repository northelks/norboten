...
| lab health (`lab-validate.yml`, `report`) | Mondays 02:00 | none | the gate on every lab × image; an issue per failed `gate (<lab>, <image>)` job unless one is open; one issue instead when more than half failed |
